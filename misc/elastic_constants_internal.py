...
def S_np(LMarg, h, X_n, X_p, check_pos_def=True,
    ...
    ai = ( "ai", 0  );   aj = ( "aj", 0  );   ak = ( "ak", 0  )
    arr = [ai, aj, ak]

    # Second atom displacements
    aii = ( "aii", 0  ); ajj = ( "ajj", 0  ); akk = ( "akk", 0  )
    arr2 = [aii, ajj, akk]
    ...
    if use_forces:
        m_11 = np.zeros((3,3));  m_12 = np.zeros((3,3));  m_21 = np.zeros((3,3));  m_22 = np.zeros((3,3))
        ...
        for i in range(6):
            ...
            for j in range(6):
                a2 = a[j]
                Phi[i,j] = cds_fourth_order(args, a1, a2, h, second_order=second_order )
                print(Phi[i,j])
...
def get_min_lp( phase = "hcp"):
    ...
    try:
        ret = minimize(fnc, x0, method='Nelder-Mead', options={'disp' : True, 'fatol' : 1e-10} )
    except TypeError:
...
def cds_fourth_order(args, ai, aj, h, second_order=False):
    ...
    for i, ni in enumerate(n_disp):
        for j, nj in enumerate(n_disp):
            xargs = args + " -v" + ai_name + "=" + str(  ni * h )  + " -v" + aj_name + "=" + str( nj * h )
            print(xargs)
            f_arr[i,j]  = find_energy(xargs, '',  'cds_fourth_order' )
            del_h_dict[(ni,nj)] = (i,j)

    fa = lambda ni, nj: f_arr[ del_h_dict[ (ni, nj) ] ]
    if second_order:
        mixed_derivative = ( 1./(4. * h**2) ) * ( fa( -1,-1) + fa(1,1) - fa(1, -1) - fa(-1, 1) )
    else:
...
def cds_second_order(args, ai, h):
    ...
    forces_atom_p1 = np.zeros(3);  forces_atom_p2 = np.zeros(3)
    forces_atom_m1 = np.zeros(3);  forces_atom_m2 = np.zeros(3)
    ...
    for j in [-1,1]:
        ...
        for i in range(3):
            greparg = " grep -A4 'Forces on atom' " + filename + " | grep 'Total' | awk '{print$" + str(i + 3) + "}'"
            if j == -1:
                forces_atom_m1[i], forces_atom_m2[i] = tuple( [ float(x) for x in ( cmd_result( greparg ).strip('\n') ).split() ] )
            else:
                forces_atom_p1[i], forces_atom_p2[i] = tuple( [ float(x) for x in ( cmd_result( greparg ).strip('\n')).split() ] )
...
ahcp=5.5125; chcp=8.8090; npts=80
...
print( 'Targets  : a=%.4f, c=%.4f c/a=%.4f' % (a_hcp_exp, c_hcp_exp, c_hcp_exp/a_hcp_exp) )
...
print( "shear constants: c_11=%5.1f, c_33=%5.1f, c_44=%5.1f, c_12=%5.1f, c_13=%5.1f, c_p= %5.1f, c_pp=%5.1f, S=%5.1f, R=%5.1f, H=%5.1f " % (c_11,c_33,c_44,c_12,c_13,c_p,c_pp,S,R,H) )
...
print( is_positive_definite( c_11_diff_min, c_33_diff_min, c_44_diff_min, c_12_diff_min, c_13_diff_min)  )
print( is_stability_satisfied( c_11_diff_min, c_33_diff_min, c_44_diff_min, c_12_diff_min, c_13_diff_min)  )
